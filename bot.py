...
class AnnouncementView(discord.ui.View):
    ...
    async def rules_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("Check the rules channel first.", ephemeral=True)

    @discord.ui.button(label="Updates", style=discord.ButtonStyle.primary, emoji="📢")
    async def updates_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("Stay tuned for more updates.", ephemeral=True)

    @discord.ui.button(label="Support", style=discord.ButtonStyle.success, emoji="🎫")
    async def support_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("Need help? Open a ticket.", ephemeral=True)

# ===== ANNOUNCE COMMAND =====
@bot.tree.command(name="announce", description="Send a clean announcement")
@app_commands.describe(
    message="Announcement message",
    title="Optional title",
    channel="Target channel",
    everyone="Ping @everyone? true or false",
    buttons="Add buttons? true or false",
    color_hex="Embed color like #5865F2",
...
async def announce(
    ...
    if not interaction.user.guild_permissions.manage_messages:
        await interaction.response.send_message("Wala kang permission para dito.", ephemeral=True)
        return

    target = channel or interaction.channel

    hex_color = color_hex.strip().replace("#", "")
    try:
        if len(hex_color) == 6:
    ...
    embed_text = message if not title else f"**{title}**\n\n{message}"
    ...
    view = AnnouncementView() if buttons else None
